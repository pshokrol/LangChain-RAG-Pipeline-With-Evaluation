...
# IMPROVED tool descriptions - more specific and clear
@tool
def SearchSimilarTickets(query: str) -> str:
    """Use this for troubleshooting questions like how to fix X, why is X happening,
    or what issues have we seen with X. Searches semantically - finds related tickets
    even if words do not match exactly. Do NOT use for ticket IDs or statistics."""
    results = vector_store.similarity_search(query, k=3)
    return "\n\n".join([doc.page_content for doc in results])

@tool
def GetTicketByID(ticket_id: str) -> str:
    """Use this ONLY when the user mentions a specific ticket ID like TICK-001 or TICK-005.
    Returns full details of that exact ticket. Do NOT use for general questions."""
...
def GetTicketStatistics(query: str) -> str:
    """Use this ONLY when user asks for counts, totals, or statistics like
    how many tickets, ticket breakdown, or category summary. Do NOT use for troubleshooting."""
    categories = {}
    for t in tickets:
...
test_queries = [
    ("What database problems have we seen?",    "SearchSimilarTickets"),
    ("Users can not log in",                    "SearchSimilarTickets"),
    ("Get TICK-010 details",                    "GetTicketByID"),
    ("How many tickets in each category?",      "GetTicketStatistics"),
]

print("=" * 60)
print("MODULE 6 PART 3: Improved Tool Descriptions")
print("=" * 60)
